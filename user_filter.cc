...
namespace {
  ...
  int my_fgets(FILE *const f, std::string &line, int &linenum) {
    ...
    while (i > 0 && UCS::isspace((unsigned char)line[i - 1]))
      --i;
    if (i < line.size())
      line.resize(i);
    return line.size();
...
bool User_filter::enable_char(const int code, int new_code) {
  if (code < 0 || code > 0xFFFF)
    return false;
  if (new_code < 0 || new_code > 0xFFFF)
    new_code = code;
  if (code < 256)
    table1[code] = new_code;
  else {
    unsigned i = table2.size();
    while (i > 0 && table2[i - 1].code > code)
      --i;
    if (i == 0 || table2[i - 1].code < code)
      table2.insert(table2.begin() + i, Entry(code, new_code));
    else
      table2[i - 1].new_code = new_code;
  }
  return true;
}
  ...
  if (i + 2 < len && line[i] == '\'') {
    if (line[i + 1] == '\\') // '\X'
    {
      ...
      if (cht >= 0 && i + 2 + l < len && line[i + 2 + l] == '\'') {
        code = UCS::map_to_ucs(cht);
        i += 3 + l;
      }
    } else if (line[i + 2] == '\'') // 'X'
    {
      code = UCS::map_to_ucs(line[i + 1]);
      i += 3;
    }
...
void User_filter::set_file_error(const char *const file_name,
                                 const int linenum) {
  char buf[80];
  ...
  while (retval_ == 0) {
    ...
    while (retval_ == 0) // parse
    {
      ...
      } else if (ch == '\'' || ch == 'U') // parse char or range
      {
        ...
        retval_ = 2;
        break;
      } // no set, char or range
      if (i < line.size() && line[i] == ',')
        ++i;
    }
  }
  if (std::fclose(f) != 0 && retval_ == 0)
    retval_ = 1;
  if (retval_ != 0)
    set_file_error(file_name, linenum);
}
  ...
  if (code >= 0) {
    if (code < 256)
      ...
      for (unsigned i = 0; i < table2.size(); ++i)
        if (code == table2[i].code) {
          result = table2[i].new_code;
          break;
        }
    }
  }
  if (result < 0 && default_ == d_leave)
    result = code;
  return result;
}
