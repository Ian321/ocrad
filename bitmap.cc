...
Bitmap::Bitmap(const Bitmap &source, const Rectangle &re)
  ...
  if (!source.includes(re))
    Ocrad::internal_error(
        "bad parameter building a Bitmap from part of another one.");
...
void Bitmap::top(const int t) {
  if (t == top())
    return;
  if (t < top())
    data.insert(data.begin(), top() - t, std::vector<uint8_t>(width(), false));
  else
...
int Bitmap::seek_left(const int row, const int col, const bool black) const {
  int c = col;
  while (c > left() && get_bit(row, c - 1) != black)
    --c;
  return c;
}
...
int Bitmap::seek_top(const int row, const int col, const bool black) const {
  int r = row;
  while (r > top() && get_bit(r - 1, col) != black)
    --r;
  return r;
}
...
int Bitmap::seek_right(const int row, const int col, const bool black) const {
  int c = col;
  while (c < right() && get_bit(row, c + 1) != black)
    ++c;
  return c;
}
...
int Bitmap::seek_bottom(const int row, const int col, const bool black) const {
  int r = row;
  while (r < bottom() && get_bit(r + 1, col) != black)
    ++r;
  return r;
}
...
bool Bitmap::escape_left(int row, int col) const {
  ...
  for (d = row; d < bottom() - 1; ++d)
    if (get_bit(d + 1, col))
      break;
  while (u <= d && --col >= left()) {
    if (u > top() + 1 && !get_bit(u, col))
      --u;
    if (d < bottom() - 1 && !get_bit(d, col))
      ++d;
    while (u <= d && get_bit(u, col))
      ++u;
    while (u <= d && get_bit(d, col))
      --d;
  }
  return (col < left());
}
...
bool Bitmap::escape_top(int row, int col) const {
  ...
  for (r = col; r < right() - 1; ++r)
    if (get_bit(row, r + 1))
      break;
  while (l <= r && --row >= top()) {
    if (l > left() + 1 && !get_bit(row, l))
      --l;
    if (r < right() - 1 && !get_bit(row, r))
      ++r;
    while (l <= r && get_bit(row, l))
      ++l;
    while (l <= r && get_bit(row, r))
      --r;
  }
  return (row < top());
}
...
bool Bitmap::escape_right(int row, int col) const {
  ...
  for (d = row; d < bottom() - 1; ++d)
    if (get_bit(d + 1, col))
      break;
  while (u <= d && ++col <= right()) {
    while (u > top() + 1 && !get_bit(u, col))
      --u;
    while (d < bottom() - 1 && !get_bit(d, col))
      ++d;
    while (u <= d && get_bit(u, col))
      ++u;
    while (u <= d && get_bit(d, col))
...
int Bitmap::follow_top(int row, int col) const {
  ...
  for (c = col; c > left() && get_bit(row, c - 1); --c)
    ;
  if (c > left())
    array.resize(c - left(), false);
  for (c = col; c < right() && get_bit(row, c + 1); ++c)
    ;
  array.resize(c - left() + 1, true);
  if (c < right())
    array.resize(width(), false);

  while (--row >= top()) {
    bool alive = false;
    for (int i = 0; i < width(); ++i)
      if (array[i]) {
        if (!get_bit(row, left() + i))
          array[i] = false;
        else
          alive = true;
      }
    ...
    for (int i = 1; i < width(); ++i)
      if (array[i - 1] && !array[i] && get_bit(row, left() + i))
        array[i] = true;
...
bool Bitmap::bottom_hook(int *hdiff) const {
  ...
  if (row > vpos(80))
    while (--row > vcenter()) {
      ...
      const int w = r - l + 1;
      if (black_section > 2 || 10 * w <= 8 * wmax)
        break;
      if (w > wmax)
        wmax = w;
      if (black_section == 2 && rcol - lcol >= 2) {
        if (escape_bottom(row, lcol + 1))
          break;
        if (hdiff)
