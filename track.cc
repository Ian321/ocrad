...
namespace {
  ...
  void error(const char *const msg) { Ocrad::internal_error(msg); }
    ...
    if (4 * r1.height() >= 3 * mean_height &&
      ...
      if (4 * r1.height() <= 5 * mean_height &&
        ...
        if (9 * r1.height() <= 10 * mean_height &&
          ...
          val = 0;
          return (r1.height() <= r2.height()) ? 0 : 1;
        }
        if (val > 1 &&
            10 * std::abs(r1.vcenter() - r2.vcenter()) <= mean_height) {
          val = 1;
          return (r1.bottom() <= r2.bottom()) ? 0 : 1;
        }
      }
      if (val > 2 &&
          10 * std::abs(r1.vcenter() - r2.vcenter()) <= mean_height) {
  ...
  int set_l(const std::vector<Rectangle> &rectangle_vector,
    ...
    for (int i1 = 0; i1 < imax && val > 0; ++i1)
      for (int i2 = i1 + 1; i2 <= imax && i2 <= i1 + 2; ++i2) {
        int i = good_reference(rectangle_vector[i1], rectangle_vector[i2], val,
  ...
  Vrhomboid set_partial_track(const std::vector<Rectangle> &rectangle_vector) {
    ...
    if (l >= 0) {
      ...
      lcol = rectangle_vector.front().hcenter();
      lvc = mean_vcenter;
    }
    if (r >= 0) {
      ...
      rcol = rectangle_vector.back().hcenter();
      rvc = mean_vcenter;
    }
...
void Vrhomboid::left(const int l) {
  if (l > right_)
    error("left, bad parameter resizing a Vrhomboid.");
  left_ = l;
}

void Vrhomboid::right(const int r) {
  if (r < left_)
    error("right, bad parameter resizing a Vrhomboid.");
  right_ = r;
}

void Vrhomboid::height(const int h) {
  if (h <= 0)
    error("height, bad parameter resizing a Vrhomboid.");
  height_ = h;
}
...
bool Vrhomboid::includes(const Rectangle &r) const {
  if (r.left() < left_ || r.right() > right_)
    return false;
...
void Track::set_track(const std::vector<Rectangle> &rectangle_vector) {
  ...
  for (unsigned i = 0; i < rectangle_vector.size(); ++i) {
    ...
    tmp.push_back(r1);
    if (i + 1 >= rectangle_vector.size())
      last = true;
    else {
      const Rectangle &r2 = rectangle_vector[i + 1];
      if (r2.left() - r1.right() >= max_gap)
        last = true;
    }
    if (last) {
      last = false;
      data.push_back(set_partial_track(tmp));
      tmp.clear();
    }
  }
