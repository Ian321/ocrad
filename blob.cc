...
namespace {
  ...
  void delete_hole(std::vector<Bitmap *> &holep_vector,
                   std::vector<Bitmap *> &v1, std::vector<Bitmap *> &v2,
    ...
    while (--i >= 0 && holep_vector[i] != p)
      ;
    if (i < 0)
      Ocrad::internal_error("delete_hole, lost hole.");
  ...
  inline void join_holes(std::vector<Bitmap *> &holep_vector,
    ...
    while (--i >= 0 && holep_vector[i] != p2)
      ;
    if (i < 0)
      Ocrad::internal_error("join_holes, lost hole.");
    holep_vector.erase(holep_vector.begin() + i);
  ...
  void delete_outer_holes(const Rectangle &re, std::vector<Bitmap *> &holepv) {
...
Blob::Blob(const Blob &b) : Bitmap(b), holepv(b.holepv) {
...
void Blob::left(const int l) {
  ...
  if (d) {
    Bitmap::left(l);
    if (d > 0)
      delete_outer_holes(*this, holepv);
  }
}
...
void Blob::find_holes() {
  ...
  for (int row = top(); row <= bottom(); ++row) {
    ...
    for (int col = left() + 1; col < right(); ++col) {
      ...
      if (get_bit(row, col))
        new_data[dcol] = this; // black pixel
      else                     // white pixel
      {
        Bitmap *p;
