  ...
  while (first + 1 < characters()) {
    ...
    if (first < last && mg >= 0) {
      ...
      for (int i = first; i < last; ++i) {
        ...
        const int gap = c2.left() - c1.right() - 1;
        if (gap >= mwt || gap > 3 * mg || (5 * gap > 2 * mw && gap > 2 * mg) ||
            (3 * c1.width() > 2 * mw && 3 * c2.width() > 2 * mw &&
             2 * gap > mw && 5 * gap > 8 * mg)) {
          ++spaces;
          spsum += gap;
          if (insert_space(i + 1)) {
      ...
      if (spaces && nospaces) {
        const Rational th =
            (Rational(3 * spsum, spaces) + Rational(nospsum, nospaces)) / 4;
        for (int i = first; i < last; ++i) {
          const Character &c1 = character(i);
          const Character &c2 = character(i + 1);
          const int gap = c2.left() - c1.right() - 1;
          if (gap > th && insert_space(i + 1)) {
            ++i;
            ++last;
          }
        }
    ...
    if (++last < characters() && insert_space(last, true))
      ++last;
    first = last;
  }
}
...
Rational Textline::mean_gap_width(const int first, int last) const {
  ...
  if (last > first)
    return Rational(sum, last - first);
  return Rational(0);
}
...
void Textline::dprint(const Control &control, const bool graph,
  ...
  if (graph || recursive) {
    ...
    std::fprintf(control.outfile,
                 "mean height = %d, median height = %d, track segments = %d, "
                 "big initials = %d\n",
...
void Textline::apply_filter(const Filter::Type filter) {
  ...
  if (modified)
    remove_leadind_trailing_duplicate_spaces();
}
...
void Textline::join_broken_unrecognized_characters() {
  for (int i = characters() - 1; i > 0; --i)
    if (!character(i).guesses() && character(i).h_overlaps(character(i - 1)))
      delete_character(i);
}
