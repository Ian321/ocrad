...
void Character::recognize1(const Charset &charset, const Rectangle &charbox) {
  if (blobs() == 1) {
    const Blob &b = blob(0);
    if (b.holes() == 0)
      recognize110(charset, charbox);
    else if (b.holes() == 1)
      recognize111(charset, charbox);
    else if (b.holes() == 2)
      recognize112(charbox);
  } else if (blobs() == 2)
    recognize12(charset, charbox);
  else if (blobs() == 3)
    recognize13(charset, charbox);
}
...
void Character::recognize110(const Charset &charset, const Rectangle &charbox) {
  ...
  if (code) {
    if (code == '.' && b.width() > b.height() &&
        b.v_includes(charbox.vcenter())) {
      add_guess(code, 1);
      add_guess('-', 0);
      return;
    }
  ...
  if (b.height() < 5 || (b.height() < 8 && b.width() < 6) ||
      b.height() > 10 * b.width() || 5 * b.height() < b.width())
    return;
  ...
  if (charset.enabled(Charset::iso_8859_9)) {
    code = f.test_s_cedilla();
    if (code) {
      add_guess(code, 0);
      return;
    }
  }
  code = f.test_235Esz(charset);
  if (code) {
    add_guess(code, 0);
    return;
  }
  ...
  if (f.bp.minima() <= 2 &&
    ...
    if (code == 'n') // Looks for '"' or merged 'rt' or 'fl'
    {
      ...
      if (2 * f.lp[f.lp.pos(10)] > b.width() &&
        ...
        if (col <= b.hpos(95) && !b.escape_top(b.vcenter(), col) &&
            set_merged_guess('r', b.hcenter(), 't', 0))
          return;
      }
    ...
    } else if (code == 'h') // Looks for merged 'rf' or 'fi'
    {
      ...
      if (f.rp.isctip(30)) {
        set_merged_guess('f', b.hcenter(), 'i', 0);
        return;
      }
  ...
  if (f.bp.minima() == 3) {
    ...
    if (b.id(b.vcenter(), b.hcenter()) == 0 &&
        ...
        b.id(b.vcenter() + 1, b.hcenter()) == 0 &&
        b.seek_left(b.vcenter(), b.hcenter()) <=
            b.hpos(25)) { // Found merged 'rn'
      int row = b.vpos(95);
      int col = b.seek_right(row, b.left());
      col = b.seek_right(row, col + 1, false);
      col = b.seek_right(row, col + 1);
      if (col > b.left() && col < b.right() &&
          set_merged_guess('r', col, 'n', 0))
        return;
    }
    if (f.tp.minima(b.height() / 3) == 1)
      add_guess('m', 0);
    return;
  }
  if (f.bp.minima() == 4 &&
      f.tp.minima(b.height() / 3) == 1) { // Found merged 'rm'
    int row = b.vpos(95);
    int col = b.seek_right(row, b.left());
    col = b.seek_right(row, col + 1, false);
    col = b.seek_right(row, col + 1);
    if (col > b.left() && col < b.right() && set_merged_guess('r', col, 'm', 0))
      return;
  }

  if (f.tp.minima(b.height() / 4) == 3) {
    if (f.segments_in_row(b.vcenter()) == 2 &&
        f.segments_in_row(b.vpos(80)) == 3)
      return; // merged 'IX'
    int hdiff;
    if (!b.bottom_hook(&hdiff) && (f.segments_in_row(b.vcenter()) < 4 ||
...
void Character::recognize112(const Rectangle &charbox) {
  ...
  if (10 * std::abs(h2.vcenter() - h1.vcenter()) <= b.height() &&
    ...
    if (5 * std::abs(h1.bottom() - b.vcenter()) <= b.height() &&
        5 * std::abs(h2.bottom() - b.vcenter()) <= b.height() && tp.isflats() &&
        bp.minima() == 2) {
      add_guess('w', 0);
      return;
    }
  ...
  if (!h1.is_hcentred_in(b))
    return;
  if (!h2.is_hcentred_in(b))
    return;
  ...
  {
    const int w =
        b.right() - std::min(b.hcenter(), std::min(h1.hcenter(), h2.hcenter()));
  ...
  if (a1 < a2 && tp.minima() == 1) {
    if (h1.h_overlaps(h2)) {
      ...
      if (!h1.v_overlaps(h2) && h1.right() < b.hcenter() &&
          h1.top() > b.top() + h1.height()) {
        add_guess('6', 0);
        return;
      }
    }
