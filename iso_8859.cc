...
int ISO_8859::escape(const std::string &seq, const unsigned i, int *lenp) {
  ...
  switch (ch) {
    ...
    case '7':
      ch -= '0';
      while (len < 3 && i + len < seq.size() &&
             ISO_8859::isodigit(seq[i + len])) {
        ch <<= 3;
        ch += seq[i + len] - '0';
        ++len;
      }
    ...
    case 'x':
    case 'X':
      if (i + 2 >= seq.size() || ISO_8859::xvalue(seq[i + 1]) < 0 ||
          ISO_8859::xvalue(seq[i + 2]) < 0)
        return -1;
      ch = (ISO_8859::xvalue(seq[i + 1]) << 4) + ISO_8859::xvalue(seq[i + 2]);
      len = 3;
      break;
  }
  if (lenp)
