...
namespace {
  ...
  int find_space_or_hyphen(const std::vector<Character *> &cpv, unsigned i) {
    while (i < cpv.size() && !cpv[i]->maybe(' ') && !cpv[i]->maybe('-'))
      ++i;
    return i;
  }
...
void Textline::check_lower_ambiguous() {
  ...
  for (int i = big_initials(); i < characters(); ++i) {
    ...
    if (c1.guesses() == 1) {
      ...
      if (capital && !small)
        c1.insert_guess(0, std::toupper(code), 1);
    }
  }
}

void Textline::recognize2(const Charset &charset) {
  if (big_initials() >= characters())
    return;
    ...
    if (!c.guesses() && c.blobs() == 3) {
      ...
      if (Ocrad::similar(b2.height(), b3.height(), 20) && !b2.h_overlaps(b3) &&
        ...
        if (b1.height() > b2.height() && b1.height() > b3.height()) {
          ...
          if (b2.h_includes(b1.hcenter()))
            c2.shift_blobp(new Blob(b1));
          else if (b3.h_includes(b1.hcenter()))
            c3.shift_blobp(new Blob(b1));
          c2.recognize1(charset, charbox(c2));
          c3.recognize1(charset, charbox(c3));
          if (c2.guesses() && c3.guesses()) {
            c = c2;
            shift_characterp(new Character(c3));
            ++i;
          }
        }
      }
  ...
  for (int i = big_initials(); i < characters(); ++i) {
    Character &c = character(i);
    if (!c.guesses() && c.blobs() == 2 && c.blob(0).v_overlaps(c.blob(1))) {
      ...
      if ((c1.guesses() && c2.guesses()) ||
        ...
        if (!c2.maybe('.') || c2.top() > c.vcenter()) {
          shift_characterp(new Character(c2));
          ++i;
        }
      }
    }
  ...
  for (int i = big_initials(); i < characters(); ++i) {
    ...
    if (!c.guesses() && c.width() > 20 && 5 * c.width() >= 3 * c.height() &&
      ...
      for (int k = ib - 1; k >= 0; --k)
        if (c.blob(k).width() > c.blob(ib).width())
          ib = k;
      ...
      if (4 * cmin > b.height() ||
          (5 * cmin > b.height() &&
           (colmin <= b.hpos(40) || colmin >= b.hpos(60))))
        continue;
      if (colmin <= b.left() || colmin >= b.right())
        continue;
      Rectangle r1(b.left(), b.top(), colmin - 1, b.bottom());
      Rectangle r2(colmin + 1, b.top(), b.right(), b.bottom());
      Blob b1(b, r1);
      b1.adjust_height();
      if (2 * b1.height() < b.height())
        continue;
      Blob b2(b, r2);
      b2.adjust_height();
      if (2 * b2.height() < b.height())
        continue;
      b1.find_holes();
      b2.find_holes();
      Character c1(new Blob(b1));
      Character c2(new Blob(b2));
      for (int j = 0; j < c.blobs(); ++j)
        if (j != ib) {
          const Blob &bj = c.blob(j);
          if (c1.includes_hcenter(bj))
            c1.shift_blobp(new Blob(bj));
          else if (c2.includes_hcenter(bj))
            c2.shift_blobp(new Blob(bj));
        }
      c1.recognize1(charset, charbox(c1));
      c2.recognize1(charset, charbox(c2));
  ...
  for (int i = big_initials(); i < characters();) {
    if (!cpv[i]->guesses() || cpv[i]->guess(0).code >= 0) {
      ++i;
      continue;
    }
      ...
      for (int j = begin; j < characters(); ++j)
        if (j != i) {
          ...
          if (c2.guesses() >= 1) {
            ...
            if (code2 >= 128)
              continue;
            if ((std::isupper(code2) && code2 != 'B' && code2 != 'Q' &&
                 5 * c1.height() < 4 * c2.height()) ||
                (UCS::islower_small(code2) && code2 != 'r' &&
      ...
      for (int j = begin; j < characters(); ++j)
        if (j != i) {
          ...
          if (c2.guesses() >= 1) {
            ...
            if (code2 >= 128)
              continue;
            if (((std::isalpha(code2) || code2 == ':') &&
                 4 * c1.height() > 5 * c2.height()) ||
  ...
  for (int i = big_initials(); i < characters(); ++i) {
    ...
    if (c1.guesses() == 1 && c1.guess(0).code == 'i') {
      ...
      if (j >= characters() || !character(j).guesses()) {
        j = i - 1;
        if (j < big_initials() || !character(j).guesses())
          continue;
      }
      Character &c2 = character(j);
          ...
          if (j != i) {
            ...
            if (c2.guesses() >= 1) {
              ...
              if ((std::isalpha(code2) && 4 * b1.height() > 5 * c2.height()) ||
                  (std::isupper(code2) &&
                   Ocrad::similar(b1.height(), c2.height(), 10)) ||
                  (std::isalpha(code2b) && 4 * c1.height() > 5 * c2.height()) ||
                  (std::isupper(code2b) &&
                   Ocrad::similar(c1.height(), c2.height(), 10))) {
                c1.insert_guess(0, UCS::toupper(code), 1);
                break;
              }
            }
          }
          ...
          if (c2.guesses() >= 1) {
            ...
            if (UCS::isdigit(code2)) {
              if (Ocrad::similar(c1.height(), c2.height(), 10))
                c1.insert_guess(0, (code == 'l') ? '1' : '0',
                                c1.guess(0).value + 1);
              break;
            }
            if (UCS::isalpha(code2) && code2 != 'o' && code2 != 'O' &&
                code2 != 'l')
              break;
          }
        }
      ...
      if (i < characters() - 1 && character(i + 1).guesses()) {
        ...
        if (UCS::isalnum(code) || code == '.' || code == '|') {
          ...
          switch (code) {
            ...
            case 'p':
            case 'q':
            case 'y': cap = (c1.bottom() + noise <= c2.bottom()); break;
  ...
  for (int begin = big_initials(), end = begin; begin < characters();
       begin = end + 1) {
      ...
      if (i >= end && digits >= 2)
        for (i = begin; i < end; ++i) {
          ...
          if (code == 'l')
            ...
            code = 0;
          if (code)
            c.insert_guess(0, code, c.guess(0).value + 1);
        }
    }
  }
  ...
  for (int begin = big_initials(), end = begin; begin < characters();
    ...
    if (end - begin < 2 || end - begin > 4)
      continue;
    const int height = character(begin).height();
    int i;
    for (i = begin; i < end; ++i) {
      Character &c = character(i);
      if (!c.maybe('|') || !Ocrad::similar(c.height(), height, 10))
        break;
    }
    if (i >= end)
      for (i = begin; i < end; ++i) {
  ...
  for (int i = big_initials(); i < characters(); ++i) {
    Character &c = character(i);
    if (c.guesses() == 2 && c.guess(0).code == 'a' && c.guess(1).code == 'Q') {
  ...
  for (int i = big_initials(); i < characters(); ++i) {
    ...
    if (code == '|' || code == 'l') {
      ...
      if ((UCS::isupper(rcode) || UCS::isdigit(rcode)) &&
        ...
        c.insert_guess(0, 'I', 1);
        continue;
      }
      if (code == 'l')
        continue;
      if (UCS::isalpha(lcode) || UCS::isalpha(rcode)) {
        c.insert_guess(0, 'l', 1);
        continue;
      }
  ...
  if (charset.enabled(Charset::iso_8859_9))
    for (int i = big_initials(), begin = i; i < characters(); ++i) {
      ...
      if (c1.guesses()) {
        ...
        if (!capital && small)
          c1.insert_guess(0, UCS::SINODOT, 1);
      }
    }
  ...
  // separate merged 'VV'
  {
    ...
    for (int i = big_initials(); i < characters(); ++i) {
      ...
      if (mean_upper_width == 0) {
        ...
        for (int j = big_initials(); j < characters(); ++j) {
          ...
          if (cj.guesses() && UCS::isupper_normal_width(cj.guess(0).code)) {
            mean_upper_width += cj.width();
            ++count;
          }
        }
        if (count <= 0)
          break; // no characters to compare
        mean_upper_width /= count;
      }
      if (c.width() < 2 * mean_upper_width)
        continue;
      const Blob &b = c.blob(0);
      int row = b.bottom();
      while (row >= b.top() && b.id(row, b.hcenter()) == 0)
        --row;
      if (row >= b.vpos(20))
        continue;
      ...
      if (2 * b1.height() < b.height() || 2 * b2.height() < b.height() ||
          !Ocrad::similar(b1.height(), b2.height(), 10, 2))
        continue;
      Character c1(new Blob(b1));
      Character c2(new Blob(b2));
      c1.only_guess('V', 0);
      ...
      c = c1;
      ++i;
      cpv.insert(cpv.begin() + i, new Character(c2));
    }
  }

  // join the sequence '°', '/', 'o', ' ' into a '%'
  for (int i = big_initials(); i + 2 < characters(); ++i) {
    Character &c1 = character(i);
    if (c1.guesses() == 1 && c1.guess(0).code == UCS::DEG) {
      if (character(i + 1).maybe('/') && character(i + 2).maybe('o') &&
          (i + 3 >= characters() || character(i + 3).maybe(' '))) {
        c1.join(character(i + 1));
        c1.join(character(i + 2));
        delete_character(i + 2);
        delete_character(i + 1);
        c1.only_guess('%', 0);
      }
    }
