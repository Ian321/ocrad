...
namespace {
  ...
  const char *const charset_name[charsets] = {"ascii", "iso-8859-9",
                                              "iso-8859-15"};
  ...
  const T_entry T_table[] = {{"none", Transformation::none},
...
bool Ocrad::similar(const int a, const int b, const int percent_dif,
  ...
  if (percent_dif > 0 && difference <= abs_dif)
    return true;
...
bool Charset::enabled(const Value cset) const {
  if (!charset_)
    return cset == iso_8859_15; // default charset
...
bool Control::add_filter(const char *const program_name,
  ...
  for (int i = 0; F_table[i].name != 0; ++i)
    if (std::strcmp(name, F_table[i].name) == 0) {
      filters.push_back(Filter(F_table[i].type));
      return true;
    }
  if (verbosity >= 0) {
