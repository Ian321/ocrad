...
int Features::test_49ARegpq(const Rectangle &charbox) const {
  ...
  if (b.escape_right(row, col)) {
    ...
    if (vbars() == 1 && vbar(0).hcenter() > b.hcenter() && hlp.decreasing() &&
        htp.decreasing() && hwp[hwp.pos(30)] < hwp[hwp.pos(70)])
      return '4';
    ...
    if (row > b.vpos(85) && tp.ispit())
      return 'Q';
    int row2 = b.seek_bottom(row, col);
    if (row2 < b.bottom() && rp.increasing(((row + (2 * row2)) / 3) - b.top()))
      return 'g';
    if (bp.minima() == 1) {
      if (h.height() >= charbox.height())
        return 'Q';
      if (h.right() < b.hcenter() && h.bottom() < b.vcenter())
        return '2';
      return 'q';
    }
  }
...
int Features::test_4ADQao(const Charset &charset,
                          const Rectangle &charbox) const {
  const Bitmap &h = b.hole(0);
  int left_delta = h.left() - b.left(), right_delta = b.right() - h.right();

  if (!lp.ispit() && lp.isflats() && rp.ispit())
    return 'D';
  if (Ocrad::similar(left_delta, right_delta, 40) && tp.minima() == 2 &&
      bp.minima() == 2 && !rp.isconvex())
    return '#';
  if (tp.minima() == 1 && bp.minima() == 1) {
    ...
    if (row < b.bottom() - 1 && !lp.isflats() &&
      ...
      if ((2 * h.height() <= b.height() || 2 * h.width() <= b.width()) &&
          wp[h.top() - b.top()] < wp[h.bottom() - b.top()])
        return '4';
      if (!rp.ispit() && !rp.isconvex())
        return 'Q';
    }
  }
  ...
  if (tp.minima() == 1 && bp.istip() && !rp.isctip(66))
    return 'A';
  if (Ocrad::similar(left_delta, right_delta, 50)) {
    if (bp.minima() == 1 && rp.isconvex() && b.test_BD())
      return 'D';
    if (bp.minima() > 1 || rp.minima() > 1 || b.test_Q()) {
      if (4 * h.size() >= b.size() || tp.ispit() || lp.ispit())
        return 'Q';
      else
        return 0;
    }
    ...
    if (lp.istip())
      return 'n';
    if (b.vpos(80) < charbox.vcenter())
      return UCS::DEG;
    return 'o';
  }
  if (left_delta > right_delta && rp.ispit() && tp.minima() == 1 &&
      bp.minima() == 1)
    return 'D';
  if (Ocrad::similar(left_delta, right_delta, 50) &&
      (bp.minima() > 1 || rp.minima() > 1))
    return 'a';
  return 0;
}
...
int Features::test_6abd(const Charset &charset) const {
  ...
  if (3 * h.width() < b.width() &&
      (bp.minima(b.height() / 4) != 1 || tp.minima(h.vcenter() - b.top()) != 1))
    return 0;
  ...
  if (b.escape_left(row, col)) {
    ...
    if (row2 > b.top()) {
      if (charset.enabled(Charset::iso_8859_15) ||
        ...
        if (row > b.vcenter() && row3 > b.vpos(20))
          return UCS::SAACUTE;
        if (oacute1)
          return UCS::SOGRAVE;
      }
  ...
  if (b.width() > 3 * h.width() && h.top() < b.vcenter() &&
      segments_in_row(b.vcenter()) == 3 && !lp.isconvex())
    return 'm';
  int hdiff;
  if (b.top_hook(&hdiff) && hdiff > 0)
    return 's';
  return 0;
}
