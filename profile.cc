...
Profile::Profile(const Bitmap &bm_, const Type t)
    : bm(bm_), type(t), limit_(-1), max_(-1), min_(-1), mean_(-1),
      isconcave_(-1), isconvex_(-1), isflat_(-1), isflats_(-1), ispit_(-1),
      istpit_(-1), isupit_(-1), isvpit_(-1), istip_(-1) {}

void Profile::initialize() {
  switch (type) {
    case left:
      data.resize(bm.height());
      limit_ = bm.width();
      for (int row = bm.top(); row <= bm.bottom(); ++row) {
        int j = bm.left();
        while (j <= bm.right() && !bm.get_bit(row, j))
          ++j;
        data[row - bm.top()] = j - bm.left();
      }
      break;
    case top:
      data.resize(bm.width());
      limit_ = bm.height();
      for (int col = bm.left(); col <= bm.right(); ++col) {
        int j = bm.top();
        while (j <= bm.bottom() && !bm.get_bit(j, col))
          ++j;
    ...
    case right:
      ...
      for (int row = bm.top(); row <= bm.bottom(); ++row) {
        int j = bm.right();
        while (j >= bm.left() && !bm.get_bit(row, j))
          --j;
    ...
    case bottom:
      ...
      for (int col = bm.left(); col <= bm.right(); ++col) {
        int j = bm.bottom();
        while (j >= bm.top() && !bm.get_bit(j, col))
          --j;
    ...
    case height:
      ...
      for (int col = bm.left(); col <= bm.right(); ++col) {
        ...
        while (u <= d && !bm.get_bit(u, col))
          ++u;
        while (u <= d && !bm.get_bit(d, col))
          --d;
    ...
    case width:
      ...
      for (int row = bm.top(); row <= bm.bottom(); ++row) {
        ...
        while (l <= r && !bm.get_bit(row, l))
          ++l;
        while (l <= r && !bm.get_bit(row, r))
          --r;
...
bool Profile::isconcave() {
  if (isconcave_ < 0) {
    ...
    if (l > r || l < pos(25) || r > pos(75))
      return isconcave_;
    if (data[pos(10)] >= dmax || data[pos(90)] >= dmax)
      return isconcave_;
    int imax = (l + r) / 2;
...
bool Profile::isconvex() {
  if (isconvex_ < 0) {
    ...
    if (lmin >= 0 || rmax <= 0 || data[l] < 2 || data[r] < 2 ||
      ...
      return isconvex_;
    if (3 * (min_end - min_begin + 1) > 2 * samples())
      return isconvex_;
    if (2 * l >= min_begin || 2 * r <= min_end + samples() - 1)
      return isconvex_;
    if (min_begin < pos(10) || min_end > pos(90))
      return isconvex_;
...
bool Profile::ispit() {
  if (ispit_ < 0) {
    ...
    if (end < begin || end > samples() - 3)
      return ispit_;

    for (i = begin + 1; i < end; ++i)
      if (data[i] > dmin + noise)
        return ispit_;
...
bool Profile::isvpit() {
  if (isvpit_ < 0) {
    ...
    isvpit_ = (bool)(ln > 0 && 2 * wl <= samples() + 1 &&
                     wl - wn <= 2 * (level - noise));
  }
  return isvpit_;
}
  ...
  if (istip_ < 0) {
    ...
    for (int i = 1; i < tries; ++i) {
      ...
      if (data[samples() - 1 - i] < rth) {
        rth = data[samples() - 1 - i];
        end = samples() - 1 - i;
      }
    }
    if (lth >= th || rth >= th)
      return istip_;
    if (3 * lth >= 2 * range() || 3 * rth >= 2 * range())
      return istip_;
    th = std::max(lth, rth);
    int status = 0;
...
bool Profile::isltip() {
  ...
  if (begin <= noise || 2 * begin > samples())
    return false;
  return true;
}
