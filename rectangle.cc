...
void Rectangle::left(const int l) {
  if (l > right_)
    error("left, bad parameter resizing a Rectangle.");
  left_ = l;
}

void Rectangle::top(const int t) {
  if (t > bottom_)
    error("top, bad parameter resizing a Rectangle.");
  top_ = t;
}

void Rectangle::right(const int r) {
  if (r < left_)
    error("right, bad parameter resizing a Rectangle.");
  right_ = r;
}

void Rectangle::bottom(const int b) {
  if (b < top_)
    error("bottom, bad parameter resizing a Rectangle.");
  bottom_ = b;
}

void Rectangle::height(const int h) {
  if (h <= 0)
    error("height, bad parameter resizing a Rectangle.");
  bottom_ = top_ + h - 1;
}

void Rectangle::width(const int w) {
  if (w <= 0)
    error("width, bad parameter resizing a Rectangle.");
  right_ = left_ + w - 1;
}
...
bool Rectangle::includes(const Rectangle &re) const {
  return (left_ <= re.left_ && top_ <= re.top_ && right_ >= re.right_ &&
          bottom_ >= re.bottom_);
}
...
bool Rectangle::v_precedes(const Rectangle &re) const {
  if (bottom_ < re.vcenter() || vcenter() < re.top_)
    return true;
...
int Rectangle::h_distance(const Rectangle &re) const {
  if (re.right_ <= left_)
    return left_ - re.right_;
  if (re.left_ >= right_)
    return re.left_ - right_;
  return 0;
}
...
int Rectangle::v_distance(const Rectangle &re) const {
  if (re.bottom_ <= top_)
    return top_ - re.bottom_;
  if (re.top_ >= bottom_)
    return re.top_ - bottom_;
  return 0;
}
