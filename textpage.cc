...
namespace {
  ...
  void Zone::join(Zone &z) {
    ...
    blobp_vector.insert(blobp_vector.end(), z.blobp_vector.begin(),
                        z.blobp_vector.end());
  ...
  void bprint(const std::vector<Zone> &zone_vector, FILE *const outfile) {
    ...
    for (unsigned zindex = 0; zindex < zone_vector.size(); ++zindex) {
      ...
      const std::vector<Blob *> &blobp_vector =
          zone_vector[zindex].blobp_vector;
  ...
  inline void join_blobs(std::vector<Blob *> &blobp_vector,
                         std::vector<Blob *> &v1, std::vector<Blob *> &v2,
    ...
    if (p1->top() > p2->top()) {
      ...
      std::replace(v1.begin() + i, v1.end(), p2, p1);

    i = blobp_vector.size();
    while (--i >= 0 && blobp_vector[i] != p2)
      ;
    if (i < 0)
      Ocrad::internal_error("join_blobs, lost blob.");
  ...
  void ignore_small_blobs(std::vector<Blob *> &blobp_vector) {
    ...
    for (int from = 0; from < blobs; ++from) {
      ...
      if (p->height() > 4 || p->width() > 4 ||
        ...
        blobp_vector[from] = blobp_vector[to];
        blobp_vector[to] = p;
        ++to;
      }
    }
    if (to < blobs) {
      for (int i = to; i < blobs; ++i)
        delete blobp_vector[i];
    ...
    for (unsigned i = 0; i < blobp_vector.size();) {
      ...
      if (4 * b.area() <= 3 * b.size()) {
        ...
        for (unsigned j = i; j < blobp_vector.size(); ++j) {
          if (blobp_vector[j]->top() > b.bottom())
            break;
          if (blobp_vector[j]->size() >= 16)
            ++blobs;
        }
        if (blobs <= b.size() / 400) {
      ...
      for (unsigned j = blobp_vector.size(); j > i;) {
        ...
        if (b.includes(b2)) {
          delete &b2;
          blobp_vector.erase(blobp_vector.begin() + j);
        }
      }
      delete &b;
    }
  ...
  int analyse_layout(std::vector<Blob *> &blobp_vector,
      ...
      for (unsigned j = 0; j < zone_vector.size(); ++j) {
        if (zone_vector[j].mask.distance(b) < 2 * mean_height) {
          if (first < 0)
            ...
            zone_vector.erase(zone_vector.begin() + j);
            --j;
          }
        }
      }
    ...
    for (unsigned i = 1; i < zone_vector.size(); ++i) {
      if (zone_vector[i].mask.top() > botmax)
        cut_index_vector.push_back(i);
    ...
    for (unsigned begin = 0, cut = 0; cut < cut_index_vector.size(); ++cut) {
      ...
      for (unsigned i = begin; i + 1 < end; ++i) {
        ...
        for (unsigned j = i + 1; j < end; ++j)
          if (zone_vector[j].mask.precedes(zone_vector[first].mask))
            first = j;
        if (first != i)
          std::swap(zone_vector[i], zone_vector[first]);
      }
      ...
      for (unsigned i = begin; join && i < end; ++i)
        if (zone_vector[i].blobp_vector.size() > 80 ||
          ...
          join = false;
      for (unsigned i = begin; join && i < end; ++i)
        if (zone_vector[i].mask.height() >
            4 * mean_blob_height(zone_vector[i].blobp_vector))
          join = false;
      if (join) {
...
Textpage::Textpage(const Page_image &page_image, const char *const filename,
  ...
  if (debug_level >= 98) {
    if (control.outfile)
      bprint(zone_vector, control.outfile);
    return;
  }
  if (debug_level > 95 || (debug_level > 89 && debug_level < 94))
    return;
  ...
  if (debug_level >= 70) {
    ...
    if ((debug_level - 70) & 1) // mark zones
      for (unsigned i = 0; i < zone_vector.size(); ++i) {
        if (debug_level == 71)
          tmp.draw_mask(zone_vector[i].mask);
        else
          tmp.draw_rectangle(zone_vector[i].mask);
      }
