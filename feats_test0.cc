    ...
    if (b.escape_left(urow2, ucol2)) {
      ...
      if (rp[lrow1 + 1 - b.top()] >= lcol1 - b.left() &&
        ...
        for (int i = lp.pos(40); i <= lp.pos(70); ++i)
          if (5 * lp[i] < b.width() && 2 * lp[i + 1] > b.width())
            return '5';
        int c = 0, hdiff;
        if (!b.top_hook(&hdiff) || 5 * hdiff >= 4 * b.height())
          ++c;
        if (2 * lp[lrow2 - b.top()] < lcol2 - b.left())
          ++c;
        if (!tp.isconvex() || (!tp.ispit() && bp.ispit()))
          ++c;
        if (c >= 2)
          return '5';
      }
      if (charset.enabled(Charset::iso_8859_15) ||
        ...
        if (urow2 > b.vpos(55) && b.seek_right(urow2 - 1, ucol2) < b.right()) {
          if (urow2 > b.vpos(63))
            return UCS::CCCEDI;
          else
            return UCS::SCCEDI;
      ...
      if (bp.minima(b.height() / 5) == 1) {
        ...
        if (rp.minima(b.width() / 8) < 3 && b.escape_bottom(urow3, ucol3)) {
          ...
          return 'F';
                   ...
                   urow2 <= b.vcenter() && lrow2 >= b.vcenter())
          return 'E';
      }
...
int Features::test_EFIJLlT(const Charset &charset,
                           const Rectangle &charbox) const {
  ...
  const int noise = (std::min(b.height(), b.width()) / 30) + 1;
  {
    ...
    if (row <= b.top() || (row < b.vpos(25) && b.escape_top(row, col))) {
      ...
      if (b.bottom_hook(&hdiff)) {
        if (maybe_j && hdiff > b.height() / 2 && rp.increasing(rp.pos(80), 1) &&
            !rp.decreasing())
          return 'J';
        if (!maybe_j && -hdiff > b.height() / 2) {
          if (5 * lp[lp.pos(80)] >= 2 * b.width())
            return 'v'; // broken 'v'
          if (col > b.hcenter())
            return 'L';
        }
  ...
  if (vbars() == 1 && vbar(0).width() >= 2 &&
    ...
    if (std::abs(vbar(0).hcenter() - b.hcenter()) <= noise &&
        std::abs((vbar(0).left() - b.left()) - (b.right() - vbar(0).right())) <=
            2 * noise) {
      ...
      if (hbars() == 2 && hbar(0).top() <= topmax &&
          4 * hbar(0).height() <= b.height() && hbar(1).bottom() >= botmin &&
          4 * hbar(1).height() <= b.height() &&
  ...
  if (vbars() == 1 && vbar(0).width() >= 2) {
    if (2 * vbar(0).width() <= b.width() && vbar(0).right() <= b.hcenter()) {
      ...
      if (hbars() == 1 && Ocrad::similar(hbar(0).width(), b.width(), 10) &&
        ...
        if (hbar(0).bottom() >= botmin && b.escape_top(b.vcenter(), b.hpos(75)))
          return 'L';
    ...
    if (3 * vbar(0).width() < 2 * b.width() && vbar(0).left() > b.hpos(33) &&
      ...
      if (vbar(0).right() >= b.hpos(90) && hbar(0).bottom() >= botmin &&
          hbar(0).left() == b.left() && b.bottom() > charbox.vpos(90) &&
  ...
  if (vbars() == 1 && vbar(0).width() >= 2 && tp.minima() == 1 &&
      bp.minima() == 1) {
    if (3 * b.height() > 4 * b.width() &&
        Ocrad::similar(vbar(0).left() - b.left(), b.right() - vbar(0).right(),
                       30, 2 * noise)) {
      if (b.height() <= 3 * wp.max() && rp.istip() && lp.istip()) {
        if (b.height() <= 3 * b.width() &&
            lp[lp.pos(40)] > lp[lp.pos(60)] + noise &&
            rp[rp.pos(60)] > rp[rp.pos(40)] + noise)
          return 'z';
        return 'I';
      }
      if (rp.isflats() &&
          (lp.istip() || lp.isflats() ||
           (lp.isctip() && lp.minima() == 2 && lp.iminimum() < lp.pos(30) &&
            lp.iminimum(1) > lp.pos(80))))
        return 'l';
      if (b.height() > 3 * wp.max()) {
        if (rp.istip() && lp.ispit() &&
            Ocrad::similar(lp.iminimum(), lp.pos(50), 10)) {
          if (lp.istpit())
            return '{';
          else
            return '(';
        ...
        if (lp.istip() && rp.ispit() &&
            Ocrad::similar(rp.iminimum(), rp.pos(50), 10)) {
          if (rp.istpit())
            return '}';
          else
            return ')';
        }
        if (rp.isflats() && 2 * vbar(0).size() >= b.area())
          return 'l';
      }
      if (2 * b.height() > 3 * b.width() && lp.minima() <= 2)
        if (rp.isflats() || rp.minima() == 1)
          if (vbar(0).right() >= b.hpos(70) ||
              b.escape_top(b.vpos(75),
                           std::min(b.right(), vbar(0).right() + 1)))
    ...
    if (vbar(0).right() >= b.right() - 1) {
      if (lp.istip() && b.height() > 2 * b.width()) {
        if (2 * vbar(0).width() <= wp.max() && lp[lp.pos(50)] >= b.width() / 2)
          return ']';
        if (b.height() >= 3 * b.width())
          return 'l';
      }
      if (2 * b.height() >= 3 * b.width() &&
          vbar(0).height() >= 3 * vbar(0).width() && lp.istpit() &&
          lp.minima() == 1) {
        const int i = lp.iminimum();
        if (i > lp.pos(10) && i < lp.pos(40))
          return '1';
      }
    }
  }
  if (hbars() == 1 && hbar(0).width() >= b.width() &&
      ...
      Ocrad::similar(b.height(), b.width(), 50) && tp.isupit() && bp.isupit())
    return '+';
  return 0;
}
...
int Features::test_c() const {
  ...
  if (b.height() > 2 * b.width() && rp.isconvex()) {
    ...
    if (3 * wp.max() <= 2 * b.width() || (2 * lp[urow - b.top()] >= b.width() &&
                                          2 * lp[lrow - b.top()] >= b.width()))
      return ')';
  }
...
int Features::test_frst(const Rectangle &charbox) const {
  ...
  if (2 * rp[rp.pos(50)] > b.width() && 2 * bp[bp.pos(50)] > b.height() &&
      tp.isctip())
    return 'r';

  if (maybe_slanted_r || vbars() != 1 || vbar(0).width() < 2)
    return 0;
  if (vbar(0).hcenter() <= b.hcenter()) {
    ...
    if (col < b.right()) {
      ...
      if (row >= b.bottom() || b.escape_bottom(row - 1, col)) {
        if (rp.minima() == 3) {
          if (rp.minima(b.width() / 8) < 3)
            return 'f';
          else
            return 0;
        }
        if (Ocrad::similar(b.height(), b.width(), 40)) {
          if (tp.minima(b.height() / 8) == 2 && bp.minima(b.height() / 8) == 2)
            return 'x';
          int row2 = b.vpos(75);
      ...
      if (!b.escape_bottom(b.vcenter(), col)) {
        ...
        if (rp.iminimum() < rp.pos(20)) {
          if (rp.increasing(rp.pos(20)) || bp.increasing() ||
              tp.minima(noise) == 2 ||
              (rp.minima() == 1 &&
               (b.height() < charbox.height() || tp.iminimum() > tp.pos(50)))) {
            if (b.height() <= 3 * wp.max())
              return 'r';
            else
              return 0;
          } else if (3 * b.height() >= 5 * b.width() && !rp.istip())
            return 'f';
        } else {
  ...
  if (lp.isconvex() || lp.ispit()) {
    ...
    if (row < b.bottom() && b.escape_right(row, col) &&
      ...
      for (lrow = row - 1; lrow > b.top(); --lrow)
        if (b.seek_right(lrow, b.hcenter()) >= b.right())
          break;
      for (urow = lrow - 1; urow > b.top(); --urow)
        if (b.seek_right(urow, b.hcenter()) < b.right())
          break;
      lrow += noise;
      if (lrow < row && urow > b.top()) {
        urow -= std::min(noise, (urow - b.top()) / 2);
        int uwidth =
            b.seek_left(urow, b.right()) - b.seek_right(urow, b.hcenter());
        int lwidth =
            b.seek_left(lrow, b.right()) - b.seek_right(lrow, b.hcenter());
  ...
  if (tp.minima(b.height() / 5) == 2 && tp.minima(b.height() / 4) == 2 &&
    ...
    if (2 * b.height() >= b.width() && b.height() >= 10 &&
      ...
      if (lg > 1 && bp.isvpit() && tp.minima(b.height() / 2) == 2 &&
          lp[lp.pos(75)] <= lg &&
          (!maybe_Y || 3 * wp[stem] > b.width() ||
           wp[stem] > wp[wp.pos(90)] + 1))
        return 'v';
      int hdiff;
      if (b.bottom_hook(&hdiff)) {
        if (std::abs(hdiff) <= b.height() / 8) {
          if (segments_in_row(b.vpos(30)) >= 3)
            return 'v';
          if (bp.isconvex()) {
            if (9 * wp[wp.pos(30)] > 10 * wp[wp.pos(50)] &&
                9 * wp[wp.pos(50)] > 10 * wp[wp.pos(70)])
              return 'v';
            else
              return 'u';
          }
      ...
      if (bp.ispit() && maybe_Y) {
        int row2 = b.top();
        while (row2 < b.bottom() && segments_in_row(row2) != 2)
          ++row2;
        int row1 = row2 + 1;
        while (row1 < b.bottom() && segments_in_row(row1) != 1)
          ++row1;
        if (row1 < b.bottom())
          row1 += wp[row1 - b.top()] / 4;
      ...
      if (b.escape_top(b.vpos(60), b.hcenter()) && !lp.istip() &&
          (4 * b.height() >= 3 * b.width() || segments_in_col(b.hpos(75)) <= 2))
        return 'u';
      ...
      if (lg > 1 && bp.ispit() && tp.minima(b.height() / 3) == 2)
        return 'v';
      if (lg <= 1 && 2 * (b.width() - rg - lg) < b.width() && rp.increasing() &&
          tp.minima(b.height() / 2) == 2)
        return 'v';
      return 0;
    }
    if (2 * b.height() >= b.width() && b.height() >= 9 && bp.minima() == 2 &&
        bp.isctip()) {
      ...
      if (Ocrad::similar(lg, rg, 80, 2) && 4 * lg < b.width() &&
          4 * rg < b.width()) {
        ...
        if (hbars() == 1 && 5 * (hbar(0).height() - 1) < b.height() &&
            ...
            Ocrad::similar(
                v_segment(hbar(0).vcenter(), hbar(0).hcenter()).size(),
                hbar(0).height(), 30, 2)) {
          if (9 * hbar(0).width() <= 10 * wp[wp.pos(50)])
            return 'H';
          return 0;
        }
        if (segments_in_row(b.vpos(60)) == 4 ||
            segments_in_row(b.vpos(70)) == 4) {
          if (2 * tp[tp.pos(50)] > b.height())
            return 'M';
          return 'w';
        }
        if ((vbars() <= 2 || (vbars() == 3 && b.height() >= b.width())) &&
          ...
          if (row > b.vcenter()) {
            ...
            Bitmap bm(b, r);
            int hdiff;
            if (bm.bottom_hook(&hdiff) && -2 * hdiff > bm.height())
              return 'u';
          }
          if (row > b.vpos(10) || vbars() >= 2)
            return 'N';
        }
        return 0;
      }
      if (3 * lg < 2 * rg && lg < b.width() / 4 && rg > b.width() / 4 &&
          rp.isctip() && tp.minima(b.height() / 8) == 2)
        return 'K';
      return 0;
    }
    if (bp.minima() <= 2 && 2 * b.width() > 5 * b.height())
      return '~';
    if (bp.minima() == 3 &&
...
int Features::test_hknwx(const Rectangle &charbox) const {
  ...
  if (m8 == 2 && bp.minima(b.height() / 2) == 1 &&
      ((lp.isctip() && rp.isctip()) || (lp.isconcave() && rp.isconcave())))
    return 'x';
  ...
  row = b.bottom() - row + 1;
  col += b.left();
  if (row > b.vpos(90) || row <= b.top())
    return 0;
  // FIXME follow gap up
  {
    int c = col;
    col = b.seek_right(row, col);
    if (col > c)
      --col;
  ...
  if (urow > b.vpos(20) || 3 * tp[tp.pos(60)] > b.height()) {
    ...
    if (m5 == 3 && segments_in_row(b.vcenter()) == 2 &&
        segments_in_row(b.vpos(80)) == 3)
      return 0; // merged 'IX'
    if ((m5 == 2 || m5 == 3) && tp.minima() >= 2 &&
        rp[rp.pos(25)] <= b.width() / 4 && (!lp.istpit() || rp.minima() == 1))
      return 'w';
    if (m5 == 1 && m8 == 1 &&
        4 * tp.max(tp.pos(40), tp.pos(60)) < 3 * b.height()) {
      if (rp.isctip(66))
        return 'k';
      else
        return 'h';
  ...
  for (col = b.hcenter() - 1; col <= b.hcenter() + 1; ++col) {
    ...
    for (int row = b.top(); row <= b.bottom(); ++row) {
      ...
      if (black && !prev_black) {
        if (++black_section == 2)
          urow2 = row - 1;
        else if (black_section == 3)
          urow3 = row - 1;
        else if (black_section == 4)
          urow4 = row - 1;
      }
      prev_black = black;
    }
    ...
    for (int row = b.top(); row < b.top() + b.width(); ++row)
      for (int col = b.left(); col <= b.right(); ++col)
        if (b.get_bit(row, col))
          ++upper_area;
    if (upper_area < (b.width() - 2) * (b.width() - 2))
      return false;
    int count1 = 0, count2 = 0;
    for (int col = b.left(); col <= b.right(); ++col) {
      if (b.get_bit(b.top() + 1, col))
        ++count1;
      if (b.get_bit(b.bottom() - 1, col))
        ++count2;
    }
    if (count1 <= count2)
      return false;
...
int Features::test_easy(const Rectangle &charbox) const {
  ...
  if (b.top() >= charbox.vcenter() && test_comma())
    return ',';
  if (b.bottom() <= charbox.vcenter() && b.height() > b.width() &&
      bp.minima() == 1) {
...
int Features::test_line(const Rectangle &charbox) const {
  ...
  if (lp.minima() == 1 && rp.minima() == 1 && 2 * b.height() >= b.width() &&
    ...
    if (slope1 < 0 && slope2 < 0 && bp.minima() == 2)
      return '^';
    if (bp.minima() != 1)
      return 0;
    if (slope1 < 0 && slope2 > 0) {
      if (b.v_includes(charbox.vcenter())) {
        if (10 * b.area() < 3 * b.size())
          return '/';
        if (b.height() > 2 * b.width())
          return 'l';
        return 0;
      }
      if (b.top() >= charbox.vcenter())
        return ',';
      return '\'';
    }
    if (slope1 > 0 && slope2 < 0) {
  ...
  if (bp.minima() == 1 && 2 * b.width() >= b.height() && tp.straight(&slope1) &&
      bp.straight(&slope2)) {
...
int Features::test_solid(const Rectangle &charbox) const {
  ...
  if (b.width() >= 3 && b.height() >= 3) {
    ...
    for (int row = b.top() + vnoise; row <= b.bottom() - vnoise; ++row) {
      ...
      if (5 * holes >= b.width())
        porosity += (5 * holes) / b.width();
    }
    if (inner_area * 100 < inner_size * 70)
      return 0;
  } else {
    inner_size = 0;
    inner_area = b.area();
  ...
  if (Ocrad::similar(b.height(), wp.max(), 20, 2)) {
    ...
    if (n >= 6) {
      int d = 0;
      for (int i = 0; i < n; ++i) {
        if (b.get_bit(b.top() + i, b.left() + i))
          ++d;
        if (b.get_bit(b.top() + i, b.right() - i))
          --d;
      }
      if (2 * std::abs(d) >= n - 1)
        return 0;
  ...
  if (porosity > 1 || inner_area * 100 < inner_size * 85 ||
      (porosity && inner_area * 100 < inner_size * 95))
    return 0;
  if (b.width() > b.height()) {
