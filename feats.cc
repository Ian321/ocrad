...
Features::Features(const Blob &b_)
    : b(b_), hbar_initialized(false), vbar_initialized(false),
      lp(b, Profile::left), tp(b, Profile::top), rp(b, Profile::right),
      bp(b, Profile::bottom), hp(b, Profile::height), wp(b, Profile::width) {}
...
int Features::hbars() const {
  if (!hbar_initialized) {
    ...
    for (unsigned i = 0; i < row_scan.size(); ++i) {
      ...
      if (jmax >= 0)
        segv.push_back(row_scan[i][jmax]);
      else
        segv.push_back(Csegment());
    }
    ...
    for (int i = 0; i < b.height(); ++i) {
      ...
      switch (state) {
        case 0:
          ...
          [[fallthrough]];
        case 1:
          if (seg.size() > limit && (i <= begin || seg.overlaps(segv[i - 1]))) {
            if (seg.left < l)
              l = seg.left;
            if (seg.right > r)
              r = seg.right;
    ...
    while (hbar_.size() > 3) // remove noise hbars
    {
      ...
      for (unsigned i = 1; i < hbar_.size(); ++i)
        if (hbar_[i].width() < wmin)
          wmin = hbar_[i].width();
      for (int i = hbar_.size() - 1; i >= 0; --i)
        if (hbar_[i].width() == wmin)
          hbar_.erase(hbar_.begin() + i);
    }
  }
  return hbar_.size();
}
      ...
      switch (state) {
        case 0:
          if (count >= limit) {
            state = 3;
            begin = col;
          } else if (count * 4 >= limit * 3) {
            state = 2;
            begin = col;
          } else if (count * 3 >= limit * 2) {
            state = 1;
            begin = col;
          }
          break;
...
int Features::test_misc(const Rectangle &charbox) const {
  ...
  if (tp.minima(b.height() / 4) == 1 && bp.minima(b.height() / 4) == 1) {
    if (b.height() > 2 * b.width() && rp.increasing() && tp.decreasing() &&
        lp.iscpit(25))
      return '1';

    if (hbars() == 1 || (hbars() == 2 && hbar(1).bottom() >= b.bottom() - 1 &&
    ...
    if (5 * b.height() >= 4 * charbox.height() && b.height() > wp.max() &&
      ...
      if( hbars() == 2 && hbar(0).bottom() < b.vpos( 25 ) &&
          ...
          3 * hbar(0).width() < 4 * hbar(1).width()*/ )
        {
        if (hbar(0).right() <= hbar(1).hcenter())
          return 0;
    ...
    if ((hbars() == 2 || hbars() == 3) && hbar(0).top() <= b.top() + 1 &&
        ...
        3 * hbar(0).width() > 4 * hbar(1).width() &&
        (hbars() == 2 || (hbar(2).bottom() >= b.bottom() - 1 &&
                          3 * hbar(0).width() > 4 * hbar(2).width())))
      return 'F';
    ...
    if (b.height() > 2 * b.width() && 5 * b.height() >= 4 * charbox.height() &&
      ...
      if (5 * rp[rp.pos(50)] > 2 * b.width()) {
        ...
        if ((b.top() < charbox.top() ||
            ...
            row <= b.top())
          return 'L';
        if (row > b.top() && b.seek_bottom(b.vpos(75), b.hpos(75)) < b.bottom())
          return '[';
      }
      return '|';
    }
