...
void Page_image::read_p2(FILE *const f, const bool invert) {
  ...
  if (maxval == 0)
    throw Page_image::Error("zero maxval in pgm file.");
  ...
  for (int row = 0; row < rows; ++row)
    for (int col = 0; col < cols; ++col) {
      ...
      if (val > maxval)
        throw Page_image::Error("value > maxval in pgm file.");
      if (invert)
        val = maxval - val;
      if (maxval > 255) {
        val *= 255;
...
void Page_image::read_p5(FILE *const f, const bool invert) {
  ...
  if (maxval == 0)
    throw Page_image::Error("zero maxval in pgm file.");
  if (maxval > 255)
    throw Page_image::Error("maxval > 255 in pgm \"P5\" file.");
  maxval_ = maxval;
  threshold_ = maxval_ / 2;
  const int rows = height(), cols = width();
...
void Page_image::read_p3(FILE *const f, const bool invert) {
  const int maxval = pnm_getint(f);
  if (maxval == 0)
    throw Page_image::Error("zero maxval in ppm file.");
  maxval_ = std::min(maxval, 255);
  threshold_ = maxval_ / 2;
  const int rows = height(), cols = width();
  ...
  for (int row = 0; row < rows; ++row)
    for (int col = 0; col < cols; ++col) {
      ...
      if (!invert)
        ...
        val = maxval - std::max(r, std::max(g, b));
      if (maxval > 255) {
        val *= 255;
        val /= maxval;
      }
...
void Page_image::read_p6(FILE *const f, const bool invert) {
  ...
  if (maxval == 0)
    throw Page_image::Error("zero maxval in ppm file.");
  if (maxval > 255)
    throw Page_image::Error("maxval > 255 in ppm \"P6\" file.");
  maxval_ = maxval;
  threshold_ = maxval_ / 2;
  const int rows = height(), cols = width();
...
Page_image::Page_image(FILE *const f, const bool invert)
...

  {
    ...
    if (tmp == 0)
      throw Error("zero width in pnm file.");
    Rectangle::width(tmp);
    tmp = pnm_getint(f);
    if (tmp == 0)
      throw Error("zero height in pnm file.");
...
bool Page_image::save(FILE *const f, const char filetype) const {
  ...
  if (filetype == '1') // pbm
    ...
    for (int row = top(); row <= bottom(); ++row) {
      ...
      if (mask != 0x80)
        std::putc(byte, f); // incomplete byte at end of row
    }
