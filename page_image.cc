...
namespace {
  ...
  int absolute_pos(Rational pos, const int left, const int right) {
    ...
    if (pos >= 0) {
      if (pos <= 1)
        a = left + (pos * (right - left)).trunc();
      else
        a = left + pos.round();
    } else {
      pos = -pos;
      if (pos <= 1)
        a = right - (pos * (right - left)).trunc();
      else
        a = right - pos.round();
    }
    return a;
  ...
  void convol_n(std::vector<std::vector<uint8_t>> &data, const int scale) {
    ...
    const int height = data.size();
    const int width = data[0].size();
    if (radius < 1 || height < min_size || width < min_size)
      return;
    ...
    for (int row = 0; row < height; ++row) {
      ...
      for (int col = 0; col < width; ++col) {
        if (datarow[col] == 0) {
          ...
          const bool rt =
              row > 0 && col < width - 1 && data[row - 1][col + 1] == 0;
          const bool lb =
              row < height - 1 && col > 0 && data[row + 1][col - 1] == 0;
          const bool rb = row < height - 1 && col < width - 1 &&
                          data[row + 1][col + 1] == 0;

          if (l || t || lt || (!rt && !lb))
            new_datarow0[2 * col] = 0;
          if (r || t || rt || (!lt && !rb))
            new_datarow0[2 * col + 1] = 0;
          if (l || b || lb || (!lt && !rb))
            new_datarow1[2 * col] = 0;
          if (r || b || rb || (!rt && !lb))
            new_datarow1[2 * col + 1] = 0;
        }
      }
    }
    data.swap(new_data);
    ...
    for (int row = 0; row < height; ++row) {
      ...
      for (int col = 0; col < width; ++col) {
        ...
        const bool lb =
            row < height - 1 && col > 0 && data[row + 1][col - 1] == 0;
        const bool rb =
            row < height - 1 && col < width - 1 && data[row + 1][col + 1] == 0;
        if (datarow[col] == 0) {
          if (l || t || lt || (!rt && !lb))
            new_datarow0[col3] = 0;
          new_datarow0[col3 + 1] = 0;
          if (r || t || rt || (!lt && !rb))
            new_datarow0[col3 + 2] = 0;
          new_datarow1[col3] = new_datarow1[col3 + 1] = new_datarow1[col3 + 2] =
              0;
          if (l || b || lb || (!lt && !rb))
            new_datarow2[col3] = 0;
          new_datarow2[col3 + 1] = 0;
          if (r || b || rb || (!rt && !lb))
            new_datarow2[col3 + 2] = 0;
        } else {
          if (l && t && lt && (!rt || !lb))
            new_datarow0[col3] = 0;
          if (r && t && rt && (!lt || !rb))
  ...
  void enlarge_n(std::vector<std::vector<uint8_t>> &data, const int n) {
    ...
    for (int row = 0; row < height; ++row) {
      ...
      for (int col = 0; col < width; ++col) {
        ...
        for (int i = 0; i < n; ++i)
          new_data.back().push_back(d);
      }
      for (int i = 1; i < n; ++i)
        new_data.push_back(new_data.back());
    }
    data.swap(new_data);
  }
  ...
  void mirror_diagonal(std::vector<std::vector<uint8_t>> &data, Rectangle &re) {
      ...
      for (int i = 0, row = 0; row < rows; ++row)
        for (int col = 0; col < cols; ++col, i += 3) {
          ...
          if (!invert)
            ...
            val = maxval_ - std::max(r, std::max(g, b));
...
Page_image::Page_image(const Page_image &source, const int scale)
    : Rectangle(source), maxval_(source.maxval_),
...
void Page_image::transform(const Transformation &t) {
  switch (t.type()) {
    ...
    case Transformation::rotate90:
      ...
      mirror_top_bottom(data);
      break;
    case Transformation::rotate180:
      mirror_left_right(data);
      mirror_top_bottom(data);
      break;
    ...
    case Transformation::mirror_d1: mirror_diagonal(data, *this); break;
