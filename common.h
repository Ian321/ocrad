...
public:
  ...
  void show_error(const char *const program_name, const char *const arg) const;
};
...
struct Filter {
  ...
  explicit Filter(const User_filter *p) : user_filterp(p), type(user) {}
  explicit Filter(const Type t) : user_filterp(0), type(t) {}
};
...
struct Control {
  ...
  Control()
      : outfile(stdout), exportfile(0), debug_level(0), filetype('4'),
        utf8(false) {}
  ~Control();
