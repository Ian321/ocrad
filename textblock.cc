...
namespace {
  ...
  void join_characters(std::vector<Textline *> &tlpv) {
    for (unsigned current_line = 0; current_line < tlpv.size();
         ++current_line) {
      ...
      for (int i = 0; i < line.characters() - 1;) {
        ...
        for (int j = i + 1; j < line.characters(); ++j) {
          ...
          if (cdn->includes_hcenter(*cup) || cup->includes_hcenter(*cdn) ||
            ...
            int k;
            if (64 * c1.size() < c2.main_blob().size())
              k = i;
            else if (64 * c2.size() < c1.main_blob().size())
              k = j;
            else if (cdn == &c2) {
              c2.join(c1);
              k = i;
...
void Textblock::apply_filters(const Control &control) {
  ...
  for (unsigned f = 0; f < control.filters.size(); ++f) {
    ...
    for (int i = 0; i < textlines(); ++i) {
      ...
      for (int j = line.big_initials(); j < line.characters(); ++j)
        if (line.is_key_character(j)) {
          l = std::min(l, line.character(j).left());
          first = j;
          break;
        }
      for (int j = line.characters() - 1; j >= first; --j)
        if (line.is_key_character(j)) {
          r = std::max(r, line.character(j).right());
          last = j;
          break;
        }
      if (i == 0) {
    ...
    if (r < l || b < t)
      continue; // can't apply filter; no text
    ...
    for (int i = 0; i < textlines(); ++i) {
      ...
      for (int j = line.characters() - 1; j >= 0; --j)
        if (line.character(j).height() >= 2 * line.height() ||
            !re.includes(line.character(j).vcenter(),
                         line.character(j).hcenter())) {
          line.delete_character(j);
          modified = true;
        }
      if (modified)
        line.remove_leadind_trailing_duplicate_spaces();
    }
  }
}
...
Textblock::Textblock(const Rectangle &page, const Rectangle &block,
  ...
  for (unsigned begin = 0, end = 0; end < blobp_vector.size(); begin = end) {
    ...
    while (++end < blobp_vector.size()) {
      if (blobp_vector[end]->top() > botmax)
        break;
    ...
    for (unsigned i = begin; i < end; ++i) {
      if (blobp_vector[i]->is_abnormal())
        continue;
      unsigned h = blobp_vector[i]->height();
      if (h >= height_distrib.size())
        height_distrib.resize(h + 1);
      ++height_distrib[h];
      ++samples;
    }
    ...
    if (valid_samples)
      mean_height /= valid_samples;

    for (unsigned i = begin; i < end; ++i) {
      Blob *const p = blobp_vector[i];
      const bool a = p->is_abnormal();
      if (p->height() >= 2 * mean_height || (a && p->height() > mean_height))
        pending_tall.push_back(p);
  ...
  for (int current_line = 0; current_line < textlines() - 1;) {
    ...
    if (line1.characters() <= 2 * line2.characters() &&
      ...
      for (int i1 = 0; !joined && i1 < line1.characters(); ++i1) {
        ...
        if (2 * c1.height() >= line2.mean_height())
          continue;
  ...
  if (textlines() >= 3) {
    for (int i = 0; i + 2 < textlines(); ++i) {
      ...
      if (line2.characters() > 2 || line1.characters() < 4 ||
          line3.characters() < 4)
        continue;
      if (!Ocrad::similar(line1.height(), line3.height(), 10))
        continue;
  ...
  for (int i = 0; i < textlines(); ++i) {
    ...
    if (!l.big_initials() && l.characters() > 2) {
      ...
      if (c0.blobs() == 1 && 4 * c0.size() < c1.size() &&
          c1.left() - c0.right() > 2 * l.height() &&
...
void Textblock::recognize(const Control &control) {
  ...
  if (textlines() >= 3) {
    int min_vdistance =
        (tlpv.back()->mean_vcenter() - tlpv.front()->mean_vcenter()) /
        (textlines() - 1);
    for (int i = 0; i + 1 < textlines(); ++i) {
      ...
      if (!Ocrad::similar(line1.characters(), line2.characters(), 50) ||
          !Ocrad::similar(line1.width(), line2.width(), 30))
        continue;
      const int vdistance = line2.mean_vcenter() - line1.mean_vcenter();
      if (vdistance >= min_vdistance)
        continue;
    ...
    if (min_vdistance > 0)
      for (int i = 0; i + 1 < textlines(); ++i) {
        ...
        const Textline &line2 = *tlpv[i + 1];
        int vdistance =
            line2.mean_vcenter() - line1.mean_vcenter() - min_vdistance;
...
void Textblock::dprint(const Control &control, bool graph,
...
void Textblock::cmark(Page_image &page_image) const {
  for (int i = 0; i < textlines(); ++i)
    tlpv[i]->cmark(page_image);
}

void Textblock::lmark(Page_image &page_image) const {
  for (int i = 0; i < textlines(); ++i)
    page_image.draw_track(*tlpv[i]);
}
