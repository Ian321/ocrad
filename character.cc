...
void Character::shift_blobp(Blob *const p) {
  ...
  for (; i >= 0; --i) {
    Blob &bi = *blobpv[i];
    if (p->vcenter() > bi.vcenter())
      break;
    if (p->vcenter() == bi.vcenter() && p->hcenter() >= bi.hcenter())
      break;
  }
...
bool Character::set_merged_guess(const int code1, const int right1,
  ...
  if (blob_index < 0 || blob_index >= blobs())
    return false;
...
void Character::join(Character &c) {
  for (int i = 0; i < c.blobs(); ++i)
    shift_blobp(c.blobpv[i]);
  c.blobpv.clear();
}
...
void Character::print(const Control &control) const {
  if (guesses()) {
    if (!control.utf8) {
      ...
      if (ch)
        std::putc(ch, control.outfile);
    } else if (gv[0].code)
      std::fputs(UCS::ucs_to_utf8(gv[0].code), control.outfile);
  } else
    std::putc('_', control.outfile);
}
...
void Character::dprint(const Control &control, const Rectangle &charbox,
  ...
  for (int i = 0; i < guesses(); ++i) {
    if (gv[i].code == '\t')
      ...
      if (ch)
        std::fprintf(control.outfile, "guess '%c', confidence %d    ", ch,
                     gv[i].value);
    } else
  ...
  if (graph) {
    std::fprintf(control.outfile,
                 ...
                 top(), right(), bottom());
    std::fprintf(control.outfile,
                 "width = %d, height = %d, hcenter = %d, vcenter = %d, black "
                 "area = %d%%\n",
    ...
    if (blobs() >= 1 && blobs() <= 3) {
      ...
      Features f(b);
      std::fprintf(control.outfile, "hbars = %d, vbars = %d\n", f.hbars(),
                   f.vbars());
    }
    std::fputc('\n', control.outfile);
    ...
    for (int row = minrow; row <= maxrow; ++row) {
      ...
      if (istop)
        std::fprintf(control.outfile, "  top(%d)", row);
      if (isvc)
        std::fprintf(control.outfile, "  vcenter(%d)", row);
      if (isbot)
        std::fprintf(control.outfile, "  bottom(%d)", row);

      if (iscbtop)
        std::fprintf(control.outfile, "  box.top(%d)", row);
      if (iscbvc)
        std::fprintf(control.outfile, "  box.vcenter(%d)", row);
      if (iscbbot)
        std::fprintf(control.outfile, "  box.bottom(%d)", row);

      if (ish1top)
        std::fprintf(control.outfile, "  h1.top(%d)", row);
      if (ish1bot)
        std::fprintf(control.outfile, "  h1.bottom(%d)", row);
      if (ish2top)
        std::fprintf(control.outfile, "  h2.top(%d)", row);
      if (ish2bot)
        std::fprintf(control.outfile, "  h2.bottom(%d)", row);
...
void Character::xprint(const Control &control) const {
  std::fprintf(control.exportfile, "%3d %3d %2d %2d; %d", left(), top(),
               width(), height(), guesses());
  ...
  for (int i = 0; i < guesses(); ++i)
    if (!control.utf8 || !gv[i].code) {
      ...
      std::fprintf(control.exportfile, ", '%s'%d", UCS::ucs_to_utf8(gv[i].code),
                   gv[i].value);
  std::fputc('\n', control.exportfile);
}
...
void Character::apply_filter(const Filter::Type filter) {
  ...
  switch (filter) {
    case Filter::letters_only: remove = true; // fall through
    case Filter::letters:
      if (!UCS::isalpha(code) && !UCS::isspace(code)) {
        for (int i = 1; i < guesses(); ++i)
          if (UCS::isalpha(gv[i].code)) {
            swap_guesses(0, i);
            break;
          }
        ...
        if (remove && !UCS::isalpha(gv[0].code))
          clear_guesses();
      }
      break;
    case Filter::numbers_only: remove = true; // fall through
    case Filter::numbers:
      if (!UCS::isdigit(code) && !UCS::isspace(code)) {
        for (int i = 1; i < guesses(); ++i)
          if (UCS::isdigit(gv[i].code)) {
            swap_guesses(0, i);
            break;
          }
        if (!UCS::isdigit(gv[0].code))
          gv[0].code = UCS::to_nearest_digit(gv[0].code);
        if (remove && !UCS::isdigit(gv[0].code))
          clear_guesses();
      }
      break;
    ...
    case Filter::upper_num_only: remove = true; // fall through
    case Filter::upper_num:
      if (!UCS::isupper(code) && !UCS::isdigit(code) && !UCS::isspace(code)) {
        for (int i = 1; i < guesses(); ++i)
...
void Character::apply_user_filter(const User_filter &user_filter) {
  if (!guesses() || UCS::isspace(gv[0].code))
    return;
